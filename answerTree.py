...
hyperparams = {"depth":10, "purity_bound":0.89, "gainfunc":"gainratio"}
...
def buildTree(X: np.ndarray, Y: np.ndarray, unused: List[int], depth: int, purity_bound: float, gainfunc: Callable, prefixstr=""):
    ...
    #print(2*(hyperparams["depth"]-depth)*" ",end = '')
    #print(prefixstr, f"label {root.label} numbers {u} count {ucnt}") #可用于debug
    # 当达到终止条件时，返回叶节点
    # TODO: YOUR CODE HERE
    m,n = np.shape(X)
    if len(unused) == 0 or depth==0 or max(ucnt)/np.sum(ucnt) > purity_bound:
        return root
    ...
    for u in ufeat:
        newX = np.array([X[i] for i in range(m) if X[i,root.featidx] == u])
        newY = np.array([Y[i] for i in range(m) if X[i,root.featidx] == u])
        branch = buildTree(newX,newY,unused,depth-1,hyperparams["purity_bound"],hyperparams["gainfunc"])
        root.children[u]=branch
